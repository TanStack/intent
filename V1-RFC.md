...
> **Implementation status (verified 2026-06-19 against `origin/main`; latest release `0.1.1`; `origin/main` also contains unreleased frontmatter-validation changes).**
>
> | Item                                                                    | Status                                                                                                                                                                                                  |
> | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
> | M3 — manifest + Agent Skills spec compliance (D20/D22)                  | 🟨 **partially on `origin/main`** (#162 shipped in `0.1.1`; #168/#169 unreleased: metadata scalar reads, leaf `name`, path-based skill identity, `validate --fix`/`--check`; manifest work not started) |
...
>
> Note: `V1-RFC.md` remains branch-only, and the `rfc` branch package code is behind `origin/main` as of 2026-06-19. Current-state claims in this RFC are checked against `origin/main`, not the stale RFC-branch package sources. The removed `intent-library` files and the stale repo-root `terminalOutput` build log are gone from `origin/main`. `0.1.1` removed the unused feedback feature and shipped metadata-scalar reads with top-level fallback; `origin/main` also has unreleased spec-frontmatter validation and `validate --fix` / `--check` work.
...
`@tanstack/intent` (`v0.1.1` latest release at this update; `origin/main` also contains unreleased frontmatter-validation changes) is the CLI that powers both sides of that contract:
...
| `intent validate`             | Maintainer       | Validates all `SKILL.md` files; `origin/main` adds `--fix` and `--check` migration modes   |
...
### How consumer discovery works today (`scanner.ts` + source policy)
...
- **Metadata-aware frontmatter reads** — `scanner.ts`, `staleness.ts`, and framework-skill validation read Intent scalar fields (`type`, `framework`, `library_version`) from `metadata.*`, with a top-level fallback for pre-migration skills. This shipped in `0.1.1` (#162).
- **Agent Skills frontmatter validation, partial D20/D22** — `origin/main` now validates spec-legal leaf `name` values, rejects non-spec top-level scalar fields (`type`, `library`, `library_version`, `framework`), derives skill identity from the directory path, and provides `intent validate --check` / `--fix` for mechanical name/scalar migrations (#168/#169, unreleased). Top-level `sources` and `requires` remain allowed until the manifest work lands.
...
**Touches:** new `lockfile.ts`, new `hash.ts`, new `commands/skills-{scan,approve,diff,update}.ts`, new `mode.ts` (frozen-mode detection), gate calls in `staleness.ts` + `utils.ts:detectGlobalNodeModules`.
...
> **Implementation status (verified 2026-06-19 against `origin/main`).** The frontmatter-compliance slice has partially landed. `0.1.1` (#162) shipped metadata-first scalar reads with top-level fallback for `type`, `framework`, and `library_version`. `origin/main` also contains unreleased changes (#168/#169) that enforce leaf `name` values, derive `SkillEntry.name` from the package-relative directory path, move scalar Intent fields under `metadata` in the templates, and add flat `intent validate --check` / `--fix`. The manifest itself, `intent skills generate-manifest`, array migration for `sources` / `requires`, centralized `secrets.ts`, and the nested `intent skills validate` command are not implemented.
...
- **M3:** manifest schema validation tests. `generate-manifest` golden-file tests over representative SKILL.md fixtures assert deterministic ordering/formatting, stable output across repeated runs, invalid path rejection, duplicate path/id rejection, missing/extra `SKILL.md` detection, per-skill manifest rejection, MCP-compatible `mcpTools[]` metadata validation, runtime-field rejection, and move/rename behavior. Round-trip with `scan` (manifest → lockfile manifestHash). **D20/D22 spec-compliance tests:** `origin/main` already covers scalar top-level keys (`type`, `library`, `library_version`, `framework`), leaf `name` validation, path-derived skill identity, non-string `metadata` values, and `validate --fix` for name/scalar migrations. Final M3 adds manifest-backed `sources` / `requires` validation, arrays-in-manifest migration, and `generate-skill` output that validates clean after structured data moves out of frontmatter.
...
- **M3:** `docs/security/manifest.md`, `docs/cli/intent-skills-validate.md`, `docs/cli/intent-skills-generate-manifest.md`. Current `origin/main` docs already cover flat `intent validate --check` / `--fix` for name/scalar migrations. Final M3 docs add the nested `intent skills validate` / `generate-manifest` surfaces, Agent Skills frontmatter compliance (D20), arrays in the manifest, and the v0-to-v1 migration path for moving existing non-spec frontmatter.
...
| D20 | Agent Skills frontmatter compliance (#116, #140)                                       | Final v1 shape: only the six spec keys at top level; Intent scalars move under `metadata`; arrays (`sources`, `requires`) move to the manifest. Current status: `0.1.1` shipped a scalar reader fallback (`metadata.*` preferred, top-level fallback) and `origin/main` now errors on scalar top-level keys with `validate --fix` / `--check` for the mechanical scalar/name migration. Structured `sources` / `requires` remain top-level until the manifest lands.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
