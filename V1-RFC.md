...
- **Allowlist-level states — absent vs empty vs wildcard (D21).** The list as a whole has three special interpretations, distinct from per-entry parsing:
  - **Absent** (`intent.skills` key not present) — legacy v0 behavior: **show all** discovered sources, plus a one-time deprecation warning that a future version will require an explicit `intent.skills[]`. This is the upgrade path for existing projects, not a steady state.
