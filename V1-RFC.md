...
> **Implementation status (verified 2026-07-24 against `main`; latest release `0.3.6`).** Secure-installer work for `0.4.0` is in flight on `feat/secure-installer-0.4` and is not merged; rows below describe `main` unless they name that branch.
>
> | Item                                                                    | Status                                                                                                                                                                                                                                                                                                                                                                               |
> | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
> | Source identity signal + matching                                       | ✅ **shipped in `0.2.1`/`0.3.5`** (#171 records `npm` vs `workspace`; #191 tightens source-policy matching to `(kind, id)`)                                                                                                                                                                                                                                                          |
> | M2 — lockfile + frozen mode                                             | ⬜ not started on `main`; 🟨 **implemented unreleased on `feat/secure-installer-0.4`** (per-skill `intent.lock`, `core/lockfile/{hash,lockfile,lockfile-state,lockfile-diff}.ts`, drift enforcement in `load` and catalogue generation). `intent skills approve`/`update` are dropped per D40 — `intent install` writes the baseline. `scan`/`diff` remain as read-only diagnostics. |
...
> | D31 down-tree PnP runtime rejection                                     | ✅ **fixed on `main`** (#188; `discovery/scanner.ts` rejects Yarn PnP runtimes resolved from inside `node_modules`)                                                                                                                                                                                                                                                                  |
> | Maintainer skill-authoring cleanup                                      | ✅ **shipped in `0.3.4`** (#190; `validate --set-version`, `generate-skill` fix, and `skill-staleness-check` rewrite to the real `intent stale` command)                                                                                                                                                                                                                             |
...
> | M7 Part B — staleness hardening (1.0 maintainer-reliability commitment) | ⬜ not started (separate perf/staleness robustness work shipped in #187 bounds the registry fetch and global package-manager detection with timeouts on the current path; it is not the M7 layered detector)                                                                                                                                                                         |
> | M7 Part A — maintainer agent surface (cut candidate, rides on M5)       | ⬜ not started                                                                                                                                                                                                                                                                                                                                                                       |
> | P1 — personal source overlay (`intent.local.json`, D25)                 | 🟦 resolved, deferred (fast-follow after the lockfile ships; not started)                                                                                                                                                                                                                                                                                                            |
> | P2 — allowlist editor + bootstrap (`intent skills add`/`init`, D24/D26) | ⬛ **superseded by D40** — `intent install` is the interactive allowlist writer, so the separate `intent skills add`/`remove`/`list`/`init` editor family is dropped (see §6, D24, D26)                                                                                                                                                                                              |
> | P3 — static install/symlink adapter for scan-based agents (D39)         | ✅ **resolved (D39)** — symlink placement is one of three delivery methods behind `intent install`; all three gates are met. Implemented unreleased on `feat/secure-installer-0.4`. See §15 P3.                                                                                                                                                                                      |
>
> Note: `V1-RFC.md` remains branch-only, and current-state claims in this RFC are checked against `main`, not the stale RFC-branch package sources. The removed `intent-library` files and the stale repo-root `terminalOutput` build log are gone from `main`. `0.1.1` removed the unused feedback feature and shipped metadata-scalar reads with top-level fallback; `0.2.0` shipped spec-frontmatter validation and `validate --fix` / `--check`; `0.2.1` shipped the scanner package-kind signal; `0.3.x` shipped the hooks/session-catalog surface, hidden-source redaction, permit-all notice hardening, PnP trust-boundary guard, maintainer skill-authoring cleanup, and `(kind, id)` source matching. The `origin/mcp` branch (last commit 2026-05-11) predates this `main` state and is **not** authoritative for the RFC's M5 path — it is not evidence that MCP work is underway (see M5). Separate perf/robustness work (PR #187) bounds the staleness registry fetch and global package-manager detection with timeouts on the current code path; it is not the M7 layered detector. M2+ lockfile, manifest, MCP, and security-doctor work remains unimplemented unless explicitly marked above.
...
`@tanstack/intent` (`v0.3.5` latest release at this update) is the CLI that powers both sides of that contract:
...
4. **Capability disclosure** — skills _declare_ what they do (read, write, network, downloads/installs, secrets, bundled scripts, MCP tools), and consumers _review_ at that granularity. These are maintainer disclosures surfaced in the diff, not limits Intent enforces (see §1 _Threat model_, D27/D28); their teeth are re-prompting on change.
...
- **Change disclosure** — any content, version, capability, manifest, or MCP-metadata change re-surfaces for review (`install` delta, `scan`, `diff`).
...
- **Content that the user did not actually read.** `intent install` shows each skill's name, source, and description — not its full text. Enabling a skill records a decision; it does not evidence a reading. Full text stays available through `intent skills diff` and by opening the file. Docs and command output must never describe the install flow as a content review.
- **Live content behind a symlink.** Symlink delivery (D39) points at the installed package's current bytes. A package update can change linked content before Intent verifies `intent.lock` again; Intent detects that drift on its next run but cannot prevent a read in between. `intent load` and catalogue generation check the lock immediately before returning content and can fail closed; direct symlinks cannot. The installer states this tradeoff and requires explicit confirmation.
...
**What "approved" means.** A source in `intent.lock` means **a human decided to enable this exact content**, not "a human read it" and not "Intent verified this skill is safe." First install displays names, sources, and descriptions; a delta run displays what changed. Neither pages through skill text. Docs, prompts, and `docs/concepts/trust-model.md` must use that meaning consistently and must not imply Intent vets content or that the user read it. Secret-scanning (M3/M6) is defense-in-depth against a literal secret _value_ leaking into skill text; it is not a content-safety check and must not be described as one.

Content-level defenses that _could_ raise this bar — skill signing/provenance, prose-level injection analysis, sandboxed execution — are explicitly deferred (§14). v1's security posture is: **trust is established by an explicit, reproducible, gated, fully-disclosed decision**, and everything in the machinery exists to make that decision honest and unavoidable, not to replace the reading it invites.

## 2. Distribution model (sets the trust boundary)

**Hard rule (preserved from prior design decisions):** library packages ship **data only** — `skills/` directory (each skill folder may contain `SKILL.md` plus spec-standard `references/`, `assets/`, and `scripts/`), manifest (M3), `keywords: ["tanstack-intent"]`. "Data only" means the package ships no Intent executable and no per-library bin or runtime Intent loads — it does **not** forbid a skill folder from containing `scripts/`. Skill-bundled `scripts/` are inert content: Intent hashes and discloses them but **never executes them** (§2 _Static discovery boundary_); if an agent runs one, that is the agent's own tool-permission surface, outside Intent (D27/D28). See §3.
...
- **Consumers** (app projects) reach Intent functionality by either:
  ...
  - Running `npx @tanstack/intent@<exact-version>` for one-off discovery (`intent list`). Not suitable for lock-driven workflows; `intent install`, `intent sync`, `intent skills scan/diff`, `intent mcp serve`, and `intent security doctor` should always run from a pinned devDep install. `intent sync` additionally runs from `prepare`, which requires the project-local binary.
...
### Distribution adapters and non-goals

Intent's canonical authored artifacts are:

- Spec-compliant `skills/**/SKILL.md`.
- `skills/intent.manifest.json` for structured package-level skill metadata.
- `intent.lock` for consumer approval, source identity, hashes, capability disclosures, and frozen-mode policy.

External discovery and distribution surfaces are adapters, not new trust roots. Intent may read from or emit to ecosystem surfaces such as `agentskills`, `agents`, `.well-known/agent-skills`, MCP `skill://` resources, GitHub or CLI installer layouts, registry manifests, `skills.json`, OCI artifacts, and agent-specific directories, but those surfaces must not become independent sources of truth in v1.

V1 implements only the adapters already scoped in this RFC: npm and workspace package discovery, three consumer-side delivery methods (symlinked skill folders, lifecycle hooks, and a compact skill map in agent instructions — D39), package manifests, lockfiles, and MCP serving from approved state.

Future adapters split by side but share one identity and hash model. **Package-side adapters** (`.well-known`, `agentskills` / `agents` package metadata, registry/catalogue metadata) derive from the authored `skills/intent.manifest.json` — they publish what a package _ships_. **Consumer-side adapters** (native agent-directory export, the MCP catalog) derive from the approved `intent.lock` — they publish only what a consumer has _reviewed_. Both reuse the same package-relative skill paths, source identities, and content hashes, so the same source skill yields the same digest across npm, docs, and MCP transports. No adapter may invent a second identity model, bypass approval, import library code, or treat a registry/catalogue entry as trust.
...
**Resolved:** `contentHash` is an aggregate hash over the normalized package-relative paths and raw bytes of **every file in a skill directory** — `SKILL.md` plus any spec-standard supporting files under `references/`, `assets/`, and `scripts/` — sorted by normalized path. Hashing the whole skill folder, not just `SKILL.md`, closes a silent-mutation hole: a changed `references/x.md` or `scripts/y.mjs` must move the hash so it re-surfaces for approval and trips frozen-mode drift, exactly as a `SKILL.md` change does.

The hash input is a deterministic sequence of entries. Each entry contains:

- A normalized package-relative path to a file inside a skill directory (`SKILL.md` or a supporting file under `references/`, `assets/`, `scripts/`).
- The file bytes read by the scanner. Text files have line endings normalized to LF before hashing (see below); binary supporting files (e.g. images under `assets/`) are hashed byte-exact, since Git applies no EOL conversion to them and they are already stable across OS.
...
> **Implementation status (D31, verified 2026-07-05 against `main`).** This guard shipped in `0.3.4` (#188). `discovery/scanner.ts` now rejects a Yarn PnP runtime resolved from inside `node_modules`, so the root-or-ancestor trust boundary is asserted in code and covered by a regression test.
...
- Skill-bundled `scripts/` — read as bytes for hashing and disclosure, never executed by Intent. Any execution is the agent's own tool-permission surface, outside Intent (D27/D28).

Allowed reads after resolution:

- `package.json`.
- `skills/intent.manifest.json`.
- Files under `skills/`, including `SKILL.md` and skill-bundled supporting files under `references/`, `assets/`, and `scripts/` (read as bytes only).
...
- **Exclude / blacklist is preserved and extended (regression guard — see §3).** The existing `package.json#intent.exclude[]` subtractive filter stays. Semantics in the allowlist world:
  ...
  - **Dedicated `intent exclude` command (shipped `0.1.0`, #157 — supersedes the original "no exclude command" plan).** Excludes are managed by a flat top-level `intent exclude` command with `add`/`remove`/`list` actions that read and write `package.json#intent.exclude[]` in place. This was a deliberate choice to give excludes a managed surface symmetric with a planned additive allowlist-management command (`intent skills add`/`remove`/`list`, D24 — later dropped by D40, which made `intent install` the allowlist writer instead) — `add`/`remove`/`list` verbs on both sides rather than hand-editing one and commanding the other. The written `exclude[]` stays declarative and PR-reviewable, so the original "reviewable in a PR like the allowlist" goal is preserved. The earlier one-off `--exclude <pattern>` flag on `list`/`load` was **removed** in the same release; one-off suppression is no longer a flag. Notice output is decoupled from warnings and can be quieted for automation via `--no-notices` (on `list`/`install`) or `INTENT_NO_NOTICES=1`.
...
- External package metadata fields such as `agentskills` or `agents` may become future adapter inputs or outputs, but they do not authorize a source in v1 unless translated into `package.json#intent.skills[]` or another reviewed Intent trust surface.

### M2 — Lockfile + delivery + frozen mode
  ...
  "sources": [
    {
      ...
      "contentHash": "sha256-...", // aggregate hash over every file in each skill folder (SKILL.md + references/assets/scripts), LF-normalized text + byte-exact binary (D32)
...
New shared modules: `lockfile.ts` (read/write/parse), `hash.ts` (sha256 helpers). Commands (revised by D40 — `approve` and `update` are dropped; `intent install` is the lockfile writer):

- `intent install` — the only command that writes `intent.lock`. Interactive, confirmed, and writes policy, lock, delivery preferences, and delivery artifacts as one operation. Refuses to run in frozen mode.
- `intent sync` — reconciles delivery against the committed lock. Noninteractive, never writes `intent.lock`. Safe in all modes.
...
- `intent install` refuses to mutate `intent.lock`. `intent sync` never writes it in any mode, so it stays usable in CI to verify that delivery matches the committed lock.
...
- One exception exists, and it belongs to M7, not M2: the staleness detector's read-only Git adapter. Its constraints are specified in M7 Part B (D29). M2 itself never shells out.

**First-run behavior (no lockfile present):**

- `intent skills scan` reports "no intent.lock — run `intent install` to create one."
- `intent install` writes the initial lockfile from the skills the user enabled during the flow.
- Frozen-mode commands refuse to run without a lockfile: "no intent.lock found; run interactively first."

#### Pre-M2 hardening (shipped on `main`)

Two shipped-M1 trust surfaces were closed before M2 implementation. M2 keeps both as regression-guard acceptance tests.
...
- **D31 — down-tree `.pnp.cjs` must be rejected.** The static-discovery trust argument holds **only** for a root-or-ancestor PnP runtime: _only a root-or-ancestor `.pnp.cjs` is trusted; a `.pnp.cjs` discovered below the project root must fail closed with a clear diagnostic._ This shipped on `main` in #188: `discovery/scanner.ts` rejects a Yarn PnP runtime resolved from inside `node_modules`, with a regression test.
...
- **`skills/sync-state.json`** — **legacy/current** staleness metadata (remote `sources_sha`, driven today by the TanStack-internal `sync-skills.mjs`). It is **superseded in principle** by lockfile content hashing once M2 (Layer 0 `contentHash`) and M7 Part B (Layer 2 baseline-ref SHAs in `intent.lock`) are complete. M2 may continue to coexist with `sync-state.json` — M2 does not touch it. **M7 owns removing or replacing `sync-state.json`** once nothing external depends on it (M7 already specifies dropping the `sync-state.json` read and moving the baseline into `intent.lock`).
  ...
  **Cleanup status:** #190 rewrote `packages/intent/meta/skill-staleness-check/SKILL.md` to describe the real current staleness mechanism (`intent stale` / `staleness/check.ts`) instead of the dead `node scripts/sync-skills.mjs <library>` reference. M2 no longer owns this cleanup.
...
- The tightening applies to **allowlist matching, exclude matching, lock entry writes, lock lookup, command-argument resolution (e.g. `intent skills diff <source>`), and drift reporting** — every surface that resolves or compares a source identity.
...
- First-run UX is documented (no-lockfile `scan` diagnostic → `intent install` flow).
...
- `skill-staleness-check` continues to point at the real `intent stale` command, not a dead `sync-skills.mjs` script (#190 regression guard).
...
**Touches:** new `core/lockfile/{lockfile,hash,lockfile-state,lockfile-diff}.ts`, new `commands/install/*` and `commands/sync/*`, new `commands/skills-{scan,diff}.ts`, frozen-mode detection, and frozen-mode gates in `staleness/check.ts` + `shared/utils.ts:detectGlobalNodeModules`. Source-policy matching, the down-tree PnP guard, and the `skill-staleness-check` cleanup have already shipped on `main`; M2 keeps them as regression guards. No CI wiring for `evals/intent-discovery/` — it stays manual (#167 / D38, revised).
...
**Hash scope (two granularities, one rule).** Each manifest `skills[]` entry carries a per-skill `contentHash` over that skill's **whole folder** — `SKILL.md` plus any `references/`, `assets/`, `scripts/` — and the lockfile source entry carries the per-package aggregate `contentHash` over every skill folder in the package. Both apply the identical canonical hashing from §2 (LF-normalized text, byte-exact binary, package-relative path rules) and differ only in scope. The per-skill folder hash is what package-side adapters consume (e.g. `.well-known` per-entry digests); the per-package aggregate is what consumer approval and diff key on. Because the hash covers supporting files, a changed reference or script re-surfaces the source for approval just like a `SKILL.md` edit.

- `intent skills generate-manifest` — walks `skills/`, computes content hashes, runs static heuristics (regex scan for `curl|wget`, `npm i|pnpm add|yarn add|bun add|pip install`, `SECRET_PATTERNS` from the shared `secrets.ts` module, fenced code blocks containing `child_process`/`spawn`/`exec`), and emits a manifest pre-filled with the heuristic findings. The maintainer reviews the diff and commits. Static analysis **informs** the manifest; the maintainer has final say on declared capabilities. Content hashing normalizes line endings to LF before hashing (D32), so a manifest generated on a CRLF working tree matches the LF bytes shipped in the tarball and the `contentHash` a consumer computes. The heuristic scan and the hash cover `SKILL.md` and skill-bundled supporting files (`references/`, `scripts/`); a non-empty `scripts/` directory pre-fills the `ships_scripts` capability for the maintainer to confirm.
...
- **Discoverable-description guidance.** A skill's `description` is the progressive-disclosure decision point — the only text an agent sees before choosing to load the full `SKILL.md` (Agent Skills spec). `generate-skill` guidance should make the `description` carry the task vocabulary and trigger phrases a developer actually uses: framework name, the operation, and the key API or file-convention terms. Autonomous discovery — the behavior the Intent effectiveness eval (`docs/intent-effectiveness-eval-plan.md`) measures — turns on description match, not body quality, so a precise skill body behind a generic description still fails to load. This is authoring guidance, not a lint: `validate` may warn on an empty or duplicated description but must not hard-fail on perceived trigger quality.
- `intent skills validate` (future nested command; today's flat `intent validate` already has `--check` and `--fix` on `main`):
  ...
  - Skill-bundled supporting files (`references/`, `assets/`, `scripts/`) resolve inside their skill directory (no `.`/`..` escape, no symlink out), are covered by that skill's folder `contentHash`, and a present non-empty `scripts/` is reflected in the skill's `capabilities` (`ships_scripts`).
...
- **Capability vocabulary (folded — no bespoke booleans).** Disclosures live in one `capabilities[]` string set on the manifest and lockfile: `reads_project_files`, `writes_project_files`, `uses_network`, `downloads_remote_content`, `runs_install_command`, `ships_scripts`. The earlier standalone `downloads` / `installs` booleans are removed — they are now `downloads_remote_content` / `runs_install_command` entries, so there is a single disclosure mechanism rather than an array plus two side booleans. `ships_scripts` is set when a skill folder contains a non-empty `scripts/`.
...
**Touches:** extends `core/lockfile/*`, `commands/skills-diff.ts`, and the `intent install` delta flow. No major new files.
...
**Catalog-scaling fallback tool (overflow path):**

- `list_skills` — compact skill index (name, package, description, capabilities summary). Registered only when the catalog exceeds configured size limits, so small/medium projects keep the single-tool path. (A dedicated `search_skills` tool is **out of v1** — D39; `list_skills` plus the agent's own filtering covers the overflow case until a real large-monorepo need is demonstrated.)

**Resolved catalog threshold behavior:** `get_skill` always exists when the lockfile is valid. Below threshold, its description embeds the full approved consumer catalog. Above threshold, `get_skill` remains available, its description contains a bounded compact summary plus guidance to call `list_skills`, and that fallback tool is registered. The fallback tool augments `get_skill`; it does not replace it.
...
`exclude[]` (M1) applies before the MCP catalog is built — excluded skills never appear in the `get_skill` description or `list_skills` results.
...
- Always runs in frozen mode. Lockfile mismatch → server starts in degraded diagnostic mode:
  ...
  - `get_skill` and `list_skills` return a structured `LOCKFILE_MISMATCH` error pointing at `get_diff` and `get_lock`.
...
- **Part A — maintainer agent surface: the cut candidate.** Part A rides on the M5 MCP server (it exposes meta-skills as MCP tools) and has no security surface of its own, so it is first to slip. If M5 runs hot, Part A fast-follows after 1.0. The CLI authoring flow (`scaffold`, `skills validate`, `skills generate-manifest`, `skills stale`) keeps working unchanged without it.

**Resolved D18 (revised):** Part B ships in v1 as a maintainer-reliability commitment; Part A is the minimal cut candidate.

Part A ships in v1 only if M1–M5 work is complete and verified without schedule risk. If it runs hot, Part A moves wholesale to fast-follow rather than shipping partially — the planned safety valve, not a failed v1. **Part B does not slip with it:** the layered staleness detector and the `intent skills stale` surface are part of the 1.0 maintainer-reliability gate.
...
M7 and the Intent effectiveness eval measure whether agents discover and use Intent surfaces. Future eval layers may score description trigger quality, overlap/conflict, stale API references, and why a relevant skill was not invoked. Those quality evals are post-v1 unless they are needed to validate an already-scoped v1 surface.
...
**Read-only Git adapter (D29).** Layer 2 is the only part of Intent that runs a subprocess in frozen mode, so its adapter carries the constraints frozen mode otherwise forbids. It is not a general subprocess escape hatch. **"Read-only git" is not safe by subcommand — git has flags that execute external programs during a read, so the adapter must constrain the entire argv, not just the subcommand.**
...
- It must place a `--` separator before any ref/path-derived value (e.g. `--baseline <ref>`), so a value beginning with `-` is parsed as a ref/path and never as a flag (argument injection).
...
intent install                    # interactive setup: method, targets, skills, lock, delivery
intent sync                       # noninteractive reconcile; runs from `prepare`
intent exclude [list|add|remove]  # manage package.json#intent.exclude (shipped 0.1.0)
intent skills scan                # read-only: what is discovered, allowed, locked
intent skills diff                # read-only: content drift against intent.lock
intent mcp serve
...
- **Resolved D40:** `intent install` and `intent sync` own every consumer write. `install` is the interactive path that writes `intent.skills`, `intent.exclude`, `intent.install`, `intent.lock`, and the delivery artifacts as one operation. `sync` is the noninteractive reconcile that repairs delivery and reports new candidates without ever writing `intent.lock`. The separate editor and lifecycle families (`intent skills add`/`remove`/`list`/`init`/`approve`/`update`) are **dropped** — they wrote the same two files through a second grammar, which is the concept duplication this RFC is trying to avoid. `intent skills scan` and `intent skills diff` survive as read-only diagnostics. This supersedes D24 and D26, and narrows D9.
- **Resolved D41:** `intent install` detects likely agent targets and pre-selects them, and the user must confirm before anything is written. Every target stays toggleable in the prompt and `--targets` bypasses detection entirely. Detection is a default, never an authorization: a pre-selected target still requires the same confirmation step as a hand-picked one. Rejected: no detection at all (the reviewability gain is invisible to the user and costs a full checkbox screen on every install, which is worse than the prevailing ecosystem behavior — see `antfu/skills-npm`).
- **Resolved D42:** `intent.skills[]` accepts skill-level entries as well as source strings. `"@tanstack/query"` allows the whole source; `{ "package": "@tanstack/query", "skills": ["fetching"] }` allows named skills only. The existing string form stays valid and unchanged. Without this, enabling 2 of 5 skills writes one allow entry plus three exclude entries — an allow decision that reads as a denylist in review. This narrows D9: skills may now be independently _allowed_, and `intent.lock` records a content baseline per skill so one changed skill does not invalidate its unchanged siblings.
...
- **`intent hooks install` is a first-class discovery surface (shipped, pre-MCP).** Alongside the future MCP server, hooks are one of the two surfaces the Intent effectiveness eval (`docs/intent-effectiveness-eval-plan.md`) measures for autonomous discovery — they exist because AGENTS-style guidance alone did not reliably make agents reach for Intent. Hooks may surface the visible skill catalog for the current session and enforce that matching guidance is loaded, but they are not an approval layer, lockfile reader, frozen-mode gate, capability review surface, or MCP replacement. M2/M5 policy still owns approval, drift refusal, and structured skill access; hooks must follow the same source-policy and future lockfile rules as `list`/`load`.
- **No allowlist editor ships (D40 supersedes D24).** `intent install` writes `intent.skills`. `intent exclude` survives because it edits a cross-cutting filter a user reaches for outside a setup flow — "stop surfacing this package" is a different moment from "set up my skills". The rejected `intent add` naming analysis lives in the D24 row.
...
## 7. Consumer first-run walkthrough
...
`intent install` asks four things and writes once:

1. **Delivery method** — symlink skill folders, install lifecycle hooks, or add a compact skill map to agent instructions.
2. **Targets** — only those the chosen method supports. Detected agents are pre-selected; the user confirms (D41).
3. **Skills** — grouped by source with descriptions. Enable all found, enable a scope pattern, or pick individually.
4. **Confirm** — then `package.json` (`intent.skills`, `intent.exclude`, `intent.install`), `intent.lock`, and the delivery artifacts are written as one operation.

After that, `prepare` keeps the installation current:

```
{ "scripts": { "prepare": "intent sync" } }
```

`intent sync` repairs delivery for unchanged accepted skills, removes stale Intent-owned artifacts, and reports new candidates. It never prompts and never writes `intent.lock`. Re-running `intent install` reviews only the delta.

```
# add to .mcp config (or equivalent) — post-M5
...
**Resolved D9, narrowed by D42:** allow decisions and content baselines are per skill; capability and manifest review stays per source.

A source listed in `intent.skills[]` may be allowed whole (`"@tanstack/query"`) or by named skill (`{ "package": "@tanstack/query", "skills": ["fetching"] }`). `intent.lock` records a content hash per skill, so a new, changed, or removed skill does not invalidate its unchanged accepted siblings from the same source. Users may still exclude individual skills through `intent.exclude`; exclusion suppresses discovery, catalog publication, MCP exposure, capability selection, generated indexes, skill lookup, and invocation.

What stays source-scoped: manifest validation, declared capabilities, declared secret names, and MCP tool metadata. Those describe the package, not one skill inside it, and a change to any of them requires review of the source as a unit.

The original D9 rejected per-skill approval because it had no demonstrated demand and no writer. The interactive installer is that writer: it displays skills individually and asks the user to decide about them individually, so recording that decision at source granularity would discard information the user already supplied.
...
- **Delivery (D39):** link reconciliation tests cover create, unchanged no-op, repair after manual deletion, removal when a source is no longer accepted, and refusal when the target path holds a real directory, an unmanaged link, or a link to a different target. Alias tests assert lossy-sanitization collisions produce deterministic unique names. Ownership tests assert cleanup requires persisted state plus exact target verification, and that missing or malformed state recreates safe links without deleting anything. `.gitignore` tests assert only the exact generated paths are managed.
- **Cross-platform:** the delivery suite runs on Linux, macOS, and Windows. Symlink semantics differ per platform in ways a single-OS suite does not catch — `fs.rmSync` with `recursive` and `force` silently leaves some directory symlinks in place, and Windows needs junctions to create and `rmdirSync` to remove them. A Linux-only matrix has already missed a real defect here.
...
- **The catalogue is the hot path, not the CLI.** Lifecycle hooks build a skill catalogue at every session and subagent start, so catalogue latency is paid far more often than any other cost in Intent. It is the only path where a regression is felt as "my agent is slow to start."
- Content hashing across a large monorepo's `node_modules` is the main cost inside that path. Verifying accepted skills against `intent.lock` must be gated on a cheap per-file `(size, mtime)` check and hash only what changed — otherwise catalogue latency scales with accepted skill _bytes_ rather than skill count. A real content edit changes mtime, so fail-closed drift detection survives the gate.
- A warm catalogue hit skips dependency discovery, `SKILL.md` parsing, and context formatting. It must still confirm the accepted skills' baselines; the cache is an optimization, never an authorization artifact.
- The cache fingerprint includes dependency lockfiles, workspace manifests, policy manifests, `intent.lock`, and a cache schema version. Cache files are untrusted local state: malformed entries, invalid paths, and schema mismatches are misses, not errors.
- `intent sync` hashes accepted skill content for drift detection and reads package metadata for new sources. It never hashes unrelated source trees, and an unchanged run performs no writes.
- The MCP server loads the lockfile once at start and only re-reads on SIGHUP (or file watcher in a follow-up).
- **Budgets, not vibes.** Cold and warm catalogue, lockfile hashing, install planning, and sync each carry a benchmark. Numeric regression budgets are established from measured baselines before merge; do not invent thresholds before collecting them.

## 12. Docs work (per milestone)

- **Cleanup + M1:** README + `docs/overview.md` + `docs/registry.md` updated for devDep-first install. `docs/concepts/trust-model.md` covers explicit sources and the static-discovery invariant. Migration guide `docs/migration/v0-to-v1.md`.
- **M2:** `docs/concepts/trust-model.md` rewritten around the four layers (discovery, policy, lock, delivery) and what the lock does _not_ prove. `docs/cli/intent-install.md` and `docs/cli/intent-sync.md` cover the setup flow, delivery methods, the symlink tradeoff, delta runs, and `prepare` wiring. `docs/cli/intent-skills.md` covers `scan`/`diff` as read-only diagnostics. Frozen-mode reference. Diagnostics docs explain best-effort parent-chain provenance as remediation context only, never transitive authorization. One page must answer "which skills can my agents use, and why is this one not showing up?" by naming the artifact responsible.
...
| D9  | Per-skill approvals                                                                              | **Narrowed by D42.** Allow decisions and content baselines are per skill; manifest, capability, declared-secret, and MCP-tool review stays source-scoped. See §8.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| D31 | Is the Yarn PnP exec exception justified, and is its trust boundary guarded?                     | **Justified, documented, and guarded on `main` (#188).** Executing the root-or-ancestor `.pnp.cjs`/`pnpapi` is acceptable because it is the same runtime the project's own `node` already loads (no new privilege); the trust argument holds only there, never for a package-supplied/down-tree PnP file. `discovery/scanner.ts` now rejects a Yarn PnP runtime resolved from inside `node_modules`, with a regression test.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| D38 | Is skill-discovery recall reliability a milestone gate or out-of-band?                           | **Revised 2026-07-02 — a manual, maintainer-run check, not a CI gate.** The `evals/intent-discovery/` harness (offline half of #167, present on `main`) remains available for a maintainer to run before/around a release; M2 does **not** wire it into CI as a blocking gate. Rationale: the harness is not in any CI workflow today, the live-Copilot half is a real subprocess and costly to run on every PR, and thresholds/flake characteristics are unvalidated — a maintainer-run check was preferred over introducing an unproven blocking surface. This reverses the original D38 text below, which called for a blocking cross-milestone CI gate; kept for history. If manual-run data later supports a pinned deterministic-only gate, that is a future decision, not part of M2. _(Superseded text, kept for audit trail: "A blocking cross-milestone gate, not out-of-band (M2 Recall-eval milestone gate, §10). The harness must stay green as a CI gate from M2 onward, with pinned thresholds and low-flake, deterministic graders. Rationale: supply-chain metadata is worthless if agents do not reliably discover/load the skills it protects. #167 should become the blocking CI gate that pins the recall/precision thresholds.")_ |

| D39 | Should Intent statically place skill files (symlink/copy) into scan-based agent directories like `.agents/skills/`? (§15 P3) | **Resolved 2026-07-24 — ship it.** Symlink placement is one of three delivery methods behind `intent install`, alongside lifecycle hooks and a compact skill map. All three gates in §15 P3 are met: per-skill `intent.lock` exists (D42); the threat model for revoked sources, occupied target paths, and hosts that cannot create symlinks is recorded in §15 P3; and acceptance criteria are listed there. Placement is lockfile-gated, ownership-tracked, and removes only links Intent created and still owns. _Original candidate framing, kept for history:_ logged as a candidate milestone (static install/symlink adapter), not designed, not scheduled, not part of M2. Intent's current distribution mechanism (`hooks/adapters.ts`) writes hook _configuration_ (e.g. `.claude/settings.json`) telling an agent to call `intent load` at runtime — it never copies `SKILL.md` files into an agent's own directory. That reaches Claude Code, Codex, and Copilot (Intent's only current adapters), but not agents that instead scan a static directory directly with no hook mechanism (Cursor, Gemini CLI, OpenCode, Junie, Goose — verified 2026-07-01 ecosystem research). A static install/symlink adapter (with a copy fallback for Windows, where symlinks need dev mode/elevated privileges) could reach that second group. **Sequenced strictly after M2**, because it needs its own trust/threat model: a symlink placed once sits statically and does not disappear when a source is later un-approved or excluded, unlike M2's per-load `isSourcePermitted`/`checkLoadAllowed` checks — so the placement step itself would need to become lockfile-aware (only place approved+locked sources, remove on un-approval) before this is safe to ship. This is an **installation** concern, distinct from discovery (M1) and activation (the hook/MCP surfaces) per the RFC's own separation of those concerns — it must not be collapsed into M2 or M3's scope. Per this RFC's standing constraint, a security-acceptance-criteria writeup is required before implementation begins, not after. |
| D40 | Do `install`/`sync` and the `intent skills` editor + lifecycle family both ship? (§6) | **No. `intent install` and `intent sync` own every consumer write; `skills add`/`remove`/`list`/`init`/`approve`/`update` are dropped.** Two command families writing the same two files (`package.json#intent.*` and `intent.lock`) through different grammars is the concept duplication this RFC exists to prevent — it doubles the docs, the failure modes, and the number of ways a project can end up in a state neither family expects. `install` writes policy, lock, delivery preferences, and delivery artifacts as one confirmed operation; `sync` reconciles delivery noninteractively and never writes `intent.lock`. `skills scan` and `skills diff` survive because they are read-only diagnostics with no writer overlap. Supersedes D24 and D26; narrows D9. |
| D41 | Should `intent install` detect installed agents and pre-select them? | **Yes — detect and pre-select, with mandatory user confirmation.** Every target stays toggleable and `--targets` bypasses detection. Detection sets a default; it never authorizes a write. Rejected: no detection at all — the reviewability it buys is invisible to the user, while the cost is a full checkbox screen on every install, worse than the prevailing ecosystem behavior (`antfu/skills-npm` detects via config directory and `PATH`, pre-selects interactively, and uses the detected set noninteractively). Rejected: detect-and-label without pre-selection — it pays the detection cost and keeps the friction. |
| D42 | Can `intent.skills[]` allow individual skills, or only whole sources? | **Individual skills.** `"@tanstack/query"` allows the source; `{ "package": "@tanstack/query", "skills": ["fetching"] }` allows named skills. The existing string form stays valid. Without this, enabling 2 of 5 skills writes one allow entry plus three exclude entries — an allow decision that reads as a denylist in review, and grows with every skill a package adds. `intent.lock` correspondingly records a content hash per skill, so one changed skill does not invalidate its unchanged accepted siblings. Manifest, capability, declared-secret, and MCP-tool review stays source-scoped because those describe the package, not one skill in it. Narrows D9. |
...
- General ecosystem adapter emission. V1 does not standardize or implement `.well-known/agent-skills`, `agentskills` / `agents` package metadata, Vercel `skills` manifests, `gh skill` layouts, `skills.json`, OCI artifacts, hosted registry publishing, or direct native-skill directory export as product commitments. These are future adapters over the canonical manifest/lock model, not v1 trust roots.
...
- Approval UI beyond the terminal setup flow. In v1, enabling a source is an explicit, confirmed CLI action that writes `intent.lock`. Hooks, MCP, and agent sessions may surface diagnostics or read-only diffs, but they must not enable sources, mutate `intent.lock`, or turn an agent chat response into approval. Web/IDE approval panels and agent-mediated approval are future work. Full-content review during first install is also out of scope — the flow shows names, sources, and descriptions, and `intent skills diff` remains the path to the bytes.
...
## 15. Proposed additions — consumer allowlist ergonomics + delivery (all resolved)

P1, P2, and P3 are all **resolved** (P1 → D25, P2 → D26, P3 → D39) and recorded in the §13
decision table. P1 remains **out of the M1 security core** (M1 shipped a read-only
`package.json#intent.skills` allowlist) and **deferred in implementation** to an independent
fast-follow after M2.

P2's chosen shape did **not** ship. D40 supersedes it: `intent install` is the interactive
allowlist writer, so the separate `intent skills add`/`remove`/`list`/`init` family is dropped.
The problem statement below is retained because it is the reason `intent install` writes
`intent.skills` at all — the gap P2 identified is real and is now closed by a different command.

P3 (added 2026-07-02, resolved 2026-07-24) ships as one of three delivery methods behind
`intent install`. Its threat model and acceptance criteria are recorded in the P3 section below,
which is what moved it from candidate to resolved.
...
independent fast-follow after M2, not part of M2.

### P2 — Allowlist bootstrap / `skills init` ergonomics gap

**Superseded by D40.** The gap is real and is now closed by `intent install`, which writes `intent.skills` interactively as part of one confirmed setup flow. Neither `intent skills init` nor the `add`/`remove`/`list` editor ships. The problem statement and security constraint below are kept because they are the reason `intent install` writes the allowlist at all.

**Original resolution (D26), superseded: ship an interactive `intent skills init` bootstrap alongside an `intent skills add`/`remove`/`list` editor (D24).** Neither ships. The command-naming analysis behind that proposal lives in the D24 row; it is not repeated here.
...
built-in permit-all is hand-writing `["*"]`. Writing `intent.lock` does **not**
fill this gap: the lock records content _from sources already matching `intent.skills`_; it does not
bootstrap `intent.skills` itself. So the gap was covered by neither M1 nor M2 as originally specced.

**Security constraint (still binding, now carried by `intent install`).** A bootstrap that auto-adds every discovered package _blindly_ defeats the
opt-in trust model. The bootstrap must be **interactive and reviewable** — show the discovered list, let
the user confirm — never a silent allow-all (that is what `["*"]` is for, with its acknowledged-risk
banner). `intent install` satisfies this: it groups discovered skills by source with descriptions, requires an explicit confirmation step, and its "enable all found" option enumerates the sources discovered during that run rather than writing a `["*"]` wildcard that would also admit unrelated future sources.

**Outcome (2026-07-24): the argument for pairing won, and one command does both.** This section originally
kept the bootstrap separate from the lockfile milestone on the grounds that writing `intent.skills` is a
trust-model change while writing `intent.lock` is reproducibility plumbing. It also recorded the
counter-argument: a fresh consumer's true first run is two steps — populate `intent.skills`, then write
`intent.lock` — sharing almost identical interactive UX, so shipping one without the other leaves a
visibly half-built onboarding. That counter-argument held. D40 collapses both writes into one confirmed
`intent install` operation, and the separation the original decision protected survives where it
matters — in the artifacts, not the command: `package.json#intent.*` records _who may contribute_,
`intent.lock` records _what content was accepted_, and each is still reviewable on its own in a diff.

### P3 — Static install/symlink adapter for scan-based agents (resolved, D39)

**Status: resolved 2026-07-24. Ships as one of three delivery methods behind `intent install`. Implemented unreleased on `feat/secure-installer-0.4`.**
...
**Resolved direction.** `intent install` offers three delivery methods over one accepted skill set: symlink skill folders, install lifecycle hooks, or write a compact skill map into agent instructions. The symlink method links approved skill folders into each selected target's skills directory using the `skills-npm` package-prefixed convention (`npm-<sanitized-package>-<skill>`, `workspace-<sanitized-package>-<skill>`), so the source identity survives in the installed name and same-named skills from different packages cannot collide. The link target is the canonical skill directory; Intent never rewrites `SKILL.md`. Windows uses directory junctions. A copy fallback is **not** a user-facing method and needs its own design before it ships.

Delivery changes how a skill reaches an agent. It never changes which skills are trusted — all three methods consume the same discovery, policy, and lock state.
...
**Non-goals.** Not a general-purpose file-copy installer for every agent layout; not a replacement for the hook adapters (both coexist — hook-capable agents can still use `intent load`); not a registry or discovery mechanism of its own.

**Threat model (gate 2).**

- _A linked source is later revoked._ `intent sync` removes the managed link when its skill was removed, excluded, or is no longer accepted. Removal is authorized by persisted ownership state plus verification that the on-disk entry is still a link to the exact target Intent recorded — never by the `npm-`/`workspace-` name prefix, which is a readability convention and not proof of ownership. If ownership state is missing or malformed, sync recreates safe current links but deletes nothing.
- _The target path is already occupied._ A real directory, an unmanaged link, or a link pointing somewhere else is a conflict. Intent leaves it untouched and reports it. `intent install` computes the full link plan as a dry run before any write and refuses to proceed while conflicts exist.
- _The host cannot create symlinks._ Windows uses directory junctions. Where neither is available, symlink delivery is unavailable and the user picks hooks or a skill map; Intent does not silently fall back to copying.
- _Live content exposure (accepted, not eliminated)._ A symlink exposes the installed package's current content. A package update can change linked content before Intent verifies `intent.lock` again. Intent detects that drift on its next run; it cannot prevent an agent from reading changed content in between. This is the one property that distinguishes symlink delivery from `intent load` and catalogue generation, both of which check the lock immediately before returning content and can fail closed. The installer states this tradeoff and requires explicit confirmation before choosing symlinks.
- _Path escape._ Content hashing and link planning reject skill paths and nested links that resolve outside the owning package root.
- _Alias collision._ Sanitizing a package name is lossy. Every alias is computed before any write; when two source identities produce the same alias, both get a deterministic short source-identity hash appended. One skill never silently replaces another.

**Acceptance criteria (gate 3).**

- Symlink mode displays the live-content tradeoff and requires confirmation before installing.
- Link destinations preserve npm versus workspace source identity.
- Alias generation detects lossy-sanitization collisions and produces deterministic unique names.
- Cleanup requires persisted ownership plus exact target verification; a name prefix alone never authorizes deletion.
- Cleanup never deletes source content, and never deletes an unmanaged entry occupying a target path.
- `intent sync` is noninteractive under `prepare`, repairs links for unchanged accepted skills, reports new candidates, and never writes `intent.lock`.
- An unchanged `intent sync` performs no writes and does not recreate current links.
- Intent adds the exact generated link paths to a managed `.gitignore` block rather than ignoring every package-prefixed name.
- Behavior is verified on Linux, macOS, and Windows. Symlink semantics differ per platform in ways that a single-OS test suite does not catch.

**Gate 1 (`intent.lock` shipped)** is met by the per-skill lockfile on `feat/secure-installer-0.4`; placement only ever links skills whose current content matches the accepted baseline.
