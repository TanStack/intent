...
V1 implements only the adapters already scoped in this RFC: npm and workspace package discovery, managed agent guidance/hooks, package manifests, lockfiles, and MCP serving from approved state.
...
**Touches:** new `lockfile.ts`, new `hash.ts`, new `commands/skills-{scan,approve,diff,update}.ts`, new `mode.ts` (frozen-mode detection), gate calls in `staleness/check.ts` + `shared/utils.ts:detectGlobalNodeModules`.
...
M3's manifest is Intent's normalized package-level skill model. Future distribution adapters must derive from it instead of reparsing `SKILL.md` frontmatter or creating transport-specific metadata as a parallel source of truth.
  ...
  "skills": [
    {
      ...
      "contentHash": "sha256-...",
      "capabilities": ["reads_project_files", "writes_project_files", "ships_scripts"],
      "declaredSecrets": [],
      "mcpTools": [
        {
          "name": "search_routes",
...
M5 is v1's first structured distribution adapter over approved Intent state. It does not make MCP the canonical distribution model; it exposes the lock-approved catalog through MCP for clients that support it.
...
| D15 | MCP tool shape and fallback threshold                                                  | `get_skill` is primary. Embed full catalog below threshold; above threshold use compact summary plus `list_skills` (no `search_skills` in v1 — D39).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| D37 | Are ecosystem discovery formats canonical or adapters?                                  | **Adapters.** Intent's canonical artifacts are `SKILL.md`, `skills/intent.manifest.json`, and `intent.lock`. External surfaces such as `agentskills`, `agents`, `.well-known/agent-skills`, MCP `skill://`, GitHub or CLI installer layouts, `skills.json`, OCI artifacts, registries, and agent-specific directories may be read or emitted by future adapters, but they do not bypass source identity, hashing, approval, or frozen-mode rules.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| D38 | Ship `references/` / `assets/` / `scripts/` supporting files, or `SKILL.md` only? | **Ship the whole skill folder.** The Agent Skills spec's progressive-disclosure model includes `references/`, `assets/`, `scripts/`; `SKILL.md`-only is too narrow. v1 hashes the **entire skill directory** (not just `SKILL.md`) so a changed reference or script re-surfaces for approval and trips frozen-mode drift — closing a silent-mutation hole. Text is LF-normalized (D32), binary is byte-exact. `scripts/` are inert data: hashed and disclosed via `capabilities` (`ships_scripts`), **never executed by Intent** (any execution is the agent's own tool-permission surface, D27/D28). MCP serves `references/` on demand; `assets/` / `scripts/` as disclosed paths. Updates §2 hashing + static-discovery boundary, M3 manifest/validate, M4 capabilities, M5. |
| D39 | Keep `search_skills` as an MCP overflow tool? | **No — cut from v1.** `get_skill` (catalog-in-description) is primary and `list_skills` covers the large-catalog overflow case; a separate text-search tool is speculative until a real large-monorepo need appears. Removing it shrinks the v1 MCP surface. It can return later behind a demonstrated catalog-scale case without a breaking change (new tool name). |
| D40 | Disclose `downloads` / `installs` as bespoke booleans or `capabilities[]` entries? | **Fold into `capabilities[]`.** The standalone `downloads` / `installs` booleans are removed in favor of `downloads_remote_content` / `runs_install_command` entries in the single `capabilities[]` set, alongside the new `ships_scripts`. One disclosure mechanism, not an array plus two side booleans; simplifies the manifest, lockfile, diff, and approval prompt. |
...
## 16. Post-v1 adapter roadmap

These adapters are intentionally outside the v1 security core. They should be sequenced only after M2/M3 provide stable identity, hashing, and manifest data.

### M8 — Distribution adapter exports

**Goal:** Publish approved, manifest-backed skills into the discovery surfaces agents and installer CLIs already use, without changing Intent's trust model.

Candidate adapters:

1. **Package metadata adapter.** Read and write unsettled JavaScript package metadata shapes such as `agentskills` and `agents` from the normalized manifest.
2. **Web discovery adapter.** Emit `/.well-known/agent-skills/index.json` and skill files with digests derived from manifest content hashes.
3. **Installer compatibility adapter.** Emit layouts or manifests compatible with common installer CLIs and GitHub-repo based skill discovery.
4. **Native agent directory adapter.** Copy or symlink approved skills into agent-specific local directories when users explicitly request it.
5. **MCP resource adapter.** If the ecosystem converges on `skill://index.json`, add a resource-oriented mode alongside the v1 tool-oriented MCP server.
6. **Registry/catalogue adapter.** Intent already has a crawl-based registry that indexes npm packages by the `tanstack-intent` keyword (`docs/registry.md`); this adapter adds a _push_ of structured, manifest-derived searchable metadata for that registry and external catalogues, without making any hosted registry authoritative for trust.
7. **OCI/provenance adapter.** Package manifest-approved skill bundles with digests/signatures once provenance semantics are designed.

When an adapter targets a name with contested ecosystem meaning — notably `skills.json`, used in the wild as both a project manifest and a package manifest — Intent detects and adapts to the consumer's existing shape rather than binding one meaning as canonical.

**Dependencies:** M2 and M3. MCP-specific adapters may also depend on M5.

### Other post-v1 candidates

- **`intent audit` (maintainer aggregate).** A single read-only command composing existing checks — `skills validate`, `skills stale`, and the discoverable-description guidance (M3) — to report which skills are missing, stale, too broad, fail spec validation, or carry weak descriptions. It adds no new trust surface; it is a reporting convenience over surfaces v1 already ships, so it stays post-v1 rather than expanding the v1 command set.
