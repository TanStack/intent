...
**Reading guide:** §0 is "state of the world today" — start here if you're not deeply familiar with the codebase. §1–4 are settled problem + context. §5–12 are the design. §13 contains the resolved decision audit trail. §16 lists narrower implementation-time open questions (which do not reopen §13).
...
> | Recall-eval harness (`evals/intent-discovery/`, #167)                   | ✅ **present on `main`** (opt-in discovery/invocation eval suite: saved-transcript scoring + optional live Copilot harness, deterministic strict-invocation graders). Promoted to a blocking milestone gate in v1 — see M2, §10, and D38 |
...
- **Recall/invocation eval harness** — `evals/intent-discovery/` is an opt-in, deterministic eval suite that measures whether an agent actually discovers and loads the right skill without being told to (saved-transcript scoring plus an optional live Copilot harness; strict-invocation graders like `StrictIntentInvocation`, `CorrectSkillLoaded`, `AutonomousDiscoverySuccess`). It exists on `main` and is the offline half of #167. v1 promotes it from an out-of-band experiment to a blocking milestone gate (see M2 and D38): supply-chain integrity is worthless if agents never load the skills it protects.
...
- `workspace:foo` does not authorize an npm `foo` of the same name (and vice versa). The narrowing is called out in the v0→v1 migration notes.
...
- **Recall-eval gate is green** (see below and D38).
...
#### Recall-eval milestone gate (applies to M2 and every later milestone)

Supply-chain metadata is necessary but not sufficient: a perfectly hashed, gated, approved skill delivers nothing if the agent never discovers or loads it. So the recall/invocation eval harness in `evals/intent-discovery/` (the offline half of #167) becomes a **blocking gate**, not an out-of-band experiment:

- The saved-transcript eval suite must stay green as a CI gate from M2 onward; #167's harness output (pinned recall/precision thresholds) gates merges once thresholds are set.
- Thresholds are **pinned** and graders are **low-flake** (deterministic strict-invocation scoring; the optional LLM judge never changes deterministic scores). A flaky gate that teams learn to ignore is itself a reliability failure.
- This is a cross-milestone gate, not a separate headline milestone — it rides on M2 and later work rather than reordering the milestone sequence.

**Touches:** new `lockfile.ts`, new `hash.ts`, new `commands/skills-{scan,approve,diff,update}.ts`, new `mode.ts` (frozen-mode detection), gate calls in `staleness/check.ts` + `shared/utils.ts:detectGlobalNodeModules`, source-policy/exclude matching promoted to `kind` + `id` (F1), a down-tree PnP guard + test in `discovery/scanner.ts` (D31), and CI wiring for the `evals/intent-discovery/` gate (#167 / D38).
...
- **Recall-eval gate (cross-milestone, D38):** the `evals/intent-discovery/` saved-transcript suite runs as a CI gate from M2 onward with pinned recall/precision thresholds and deterministic (low-flake) strict-invocation graders; the optional LLM judge never changes deterministic scores. #167's harness output is the gate. This asserts agents still discover/load the right skill as the security surface grows — metadata is not enough on its own.
...
## 16. Open questions (implementation-time)

The design decisions in §13 are resolved. These are narrower questions to answer **during** M2/M7 build — they do not reopen settled design, and each has a default the RFC leans toward.

1. **Does anything currently depend on `skills/sync-state.json`?** If yes, document a deprecation path; if no, plan its removal in M7 (D37 makes `intent.lock` the single baseline). Verify TanStack-internal `sync-skills.mjs` consumers before M7 deletes the read.
2. **What is the blast radius of the F1 `kind` + `id` tightening?** Name-only → `kind` + `id` narrows matching (e.g. `"workspace:foo"` stops authorizing an npm `foo`). Enumerate which real projects rely on the M1 permissive behavior before M2 flips it, so the v0→v1 migration note names the concrete break rather than a generic caveat.
3. **What threshold should block CI for the recall-eval harness (#167, D38)?** Pin a recall/precision floor (and a flake budget) that is high enough to be meaningful but stable enough not to red-bar merges on noise. Needs data from the existing saved-transcript corpus before wiring the gate.
4. **Should frozen mode default on under `CI=true` + non-TTY immediately, or start opt-in?** M2 currently specs auto-detect (overridable with `--no-frozen`). Confirm whether the first M2 release ships that auto-enable or begins opt-in for a transition window, so existing CI does not start hard-failing on the upgrade without warning.
5. **Should interop smoke tests cover `.agents/skills/` consumers?** If agents read skills from an `.agents/skills/` layout in addition to package `skills/` directories, decide whether v1 adds a smoke test for that consumer shape or explicitly scopes it out.
6. **How should experimental `allowed-tools` be handled while the upstream Agent Skills spec marks it experimental?** `allowed-tools` is one of the six spec top-level keys but is flagged experimental upstream. Decide whether validate treats it as pass-through, warns, or defers strict handling until the spec stabilizes — without blocking spec-compliant skills that use it.
