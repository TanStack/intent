...
- **`manifestHash`** and **`capabilities`** are reserved, always `null` today — later milestones populate them. `declaredSecrets`, `mcpTools`, and `mcpPolicy` are similarly reserved and, if present (e.g. written by a newer Intent version), are preserved on read/write but not required.
