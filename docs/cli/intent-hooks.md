...
- Builds that catalog with the project's installed `@tanstack/intent` CLI when one is resolvable from `node_modules`, and suggests `node_modules/.bin/intent load <skill-id>` for loads. Without a local install it falls back to the package-manager runner (`npx @tanstack/intent@latest`, `pnpm dlx`, ...), which resolves the package against the registry on every run.
