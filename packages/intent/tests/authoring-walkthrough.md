...
[Issue #238](https://github.com/TanStack/intent/issues/238) requests a short path while retaining the full-library process. The procedure adopts four ideas from general skill-authoring practice: conditional reference pointers with completion checks, independent skill discovery, fact lookup before dependent questions, and concrete boundary scenarios in the library's own vocabulary. No external skills, invocation flags, glossary, ADR, or provider controls became required.
