...
Run `intent maintainer setup` to create missing record skeletons without replacing surviving documents. Register a new or existing skill with `intent maintainer add`, then use `intent maintainer sync` to align tree metadata with its frontmatter and include its directory in an existing package publishing allowlist. These commands preserve authored task knowledge and decisions. Fill in missing task coverage and remove authoring markers only after doing that work; generated skeletons are incomplete records.
...
The CLI discovers an existing planning location from any Git-visible `domain_map.yaml`, `skill_spec.md`, or `skill_tree.yaml`, including custom locations, and requires all three documents there. Previously recorded locations remain required after deletion. If no location is visible or recorded, installed maintainer guidance requires the default location described above; create the records in the established custom location before reviewing a custom layout. The source-review command checks record presence, YAML shape, and review fingerprints. `maintainer check` additionally checks registered skill paths, local prerequisites, authoring markers, task entries, skill validation, and stale generated metadata. It does not prove that the inventories, decisions, or written claims are correct. Ignored files require explicit manual review.
