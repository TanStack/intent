...
The CLI discovers an existing planning location from any Git-visible `domain_map.yaml`, `skill_spec.md`, or `skill_tree.yaml`, including custom locations, and requires all three documents there. Previously recorded locations remain required after deletion. If no location is visible or recorded, installed maintainer guidance requires the default location described above; create the records in the established custom location before reviewing a custom layout. The CLI checks record presence, YAML shape, and review fingerprints. It does not prove that the inventories, decisions, or written claims are correct. Ignored files require explicit manual review.
