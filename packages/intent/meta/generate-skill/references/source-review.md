...
Read this before handing off an initial skill batch, an ordinary library change, or an Intent review reminder. The CLI identifies candidate guidance and unmapped changes from local Git evidence. It does not infer semantic impact or write skill content.
...
Review the actual diff, affected source, skill, references, and relevant tests. `changedFiles` can be empty for an initial review. A recorded content snapshot detects edits even if the earlier review happened before a commit; it does not retain the earlier source text. Use an available Git diff or explain when historical text is unavailable. Missing sources, foreign repositories, conflicts, or unavailable history remain unknown until the evidence is resolved.
...
After guidance edits and task checks, regenerate the JSON report. Save it outside tracked source paths, such as `.intent/review.json` after creating `.intent/`, so it cannot become its own review input. Annotate the selected items with:
...
Preserve the report's identity, base and fingerprints. Run `intent review --record .intent/review.json`. The command rejects stale fingerprints and unresolved source mappings. It writes completed outcomes to `.intent/review-state.json`; unresolved or unannotated items stay pending. Do not invent passing checks, use a generic reason, or mark unrelated items complete just to empty the report.
