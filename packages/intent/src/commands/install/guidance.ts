...
export function buildMaintainerGuidanceBlock(
  ...
  return {
    block: [
      ...
      `Before handing off a code change, run \`${reviewCommand}\`. Follow the maintainer procedure to update affected guidance, run task checks, and record completed review outcomes. Report an evidence-backed no-op or missing evidence explicitly.`,
