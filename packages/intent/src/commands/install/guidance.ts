...
export function buildIntentSkillGuidanceBlock(
  ...
  return {
    block: `${[
      ...
      `- If a skill clearly matches, run \`${loadCommand}\` before editing relevant files.`,
      '- If no skill clearly matches, continue normally.',
      `- If no catalogue is available in the current session, run \`${listCommand}\` once from the workspace root.`,
