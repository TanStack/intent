...
import { scanForPlaybooks } from '../src/scanner.js'
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'playbook-test-'))
})
...
describe('scanForPlaybooks', () => {
  ...
  it('returns empty packages when node_modules has no playbook packages', async () => {
  ...
  it('discovers a playbook-enabled package with skills', async () => {
  ...
  it('discovers sub-skills', async () => {
    ...
    writeJson(join(pkgDir, 'package.json'), {
      name: '@tanstack/db',
      version: '0.5.2',
      playbook: { version: 1, repo: 'TanStack/db', docs: 'docs/' },
    })
  ...
  it('warns on skills/ dir without valid playbook field', async () => {
  ...
  it('warns on invalid playbook version', async () => {
    const pkgDir = createDir(root, 'node_modules', 'wrong-ver')
    writeJson(join(pkgDir, 'package.json'), {
      name: 'wrong-ver',
      version: '1.0.0',
      playbook: { version: 99, repo: 'foo/bar', docs: 'docs/' },
    })
    createDir(pkgDir, 'skills', 'some-skill')
  ...
  it('skips packages without skills/ directory', async () => {
    ...
    writeJson(join(pkgDir, 'package.json'), {
      name: 'no-skills',
      version: '1.0.0',
      playbook: { version: 1, repo: 'foo/bar', docs: 'docs/' },
    })
    // No skills/ directory
...
describe('package manager detection', () => {
  ...
  it('throws for Yarn PnP', async () => {
    ...
    await expect(scanForPlaybooks(root)).rejects.toThrow('Yarn PnP')
  })
