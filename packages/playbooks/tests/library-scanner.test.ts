...
function shimPath(pkgDir: string): string {
  return join(pkgDir, 'bin', 'playbook.js')
}
...
describe('scanLibrary', () => {
  ...
  it('recursively discovers deps with bin.playbook', async () => {
  ...
  it('skips deps without bin.playbook', async () => {
    const pkgDir = createDir(root, 'node_modules', '@tanstack', 'router')
    writeJson(join(pkgDir, 'package.json'), {
      name: '@tanstack/router',
      version: '1.0.0',
  ...
  it('returns a warning when home package.json cannot be found', async () => {
    const fakeScript = join(root, 'nowhere', 'bin', 'playbook.js')
